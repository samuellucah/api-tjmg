...
async def ensure_input_match(page, input_locator, expected_digits: str):
    ...
    Digita e confere. Se estiver diferente (máscara cortou), apaga e tenta de novo.
    Isso resolve o problema do CNPJ cortado.
    """
    for attempt in range(3):
        try:
            ...
            # Digita dígito por dígito para forçar a máscara a se comportar
            await input_locator.type(expected_digits, delay=50)
            ...
            # Se falhou, tenta tab para validar e reseta
...
async def scrape_pje(doc_digits: str, doc_type: str) -> Dict[str, Any]:
    ...
    async with async_playwright() as p:
        ...
        try:
            ...
            # 2. Digita com Verificação (AQUI ESTÁ A CORREÇÃO DE OURO)
            # Se digitar e cortar o número, ele apaga e tenta de novo
            ...
            # Tenta pegar links diretos
            links = page.locator("a").filter(has_text=CNJ_RE)
            
            # Se não achou links diretos, tenta pegar linhas de tabela (fallback)
            ...
            for i in range(count):
                ...
                # Se for link, clica. Se for TR, busca o link dentro.
                clickable = item
